...
def generate_description_pdf(excel_path,output_path):
    ...
    for i in range(page_len):
        ...
        while  (j < cards_num[0] * cards_num[1]):
            ...
            if system == 'Darwin':
                file_name = '{}/Description PDF/description_{}.pdf'.format(output_path,i)
            else:
                file_name = '{}\\Description PDF\\description_{}.pdf'.format(output_path,i)
            ...
            for y in range(cards_num[1]):
                for x in range(cards_num[0]):
                    ...
                    for k in range(len(description_list)):
                        ...
                        description_width_list.append(round(page.stringWidth(description_list[k],'Meiryo UI',25)))

                        x_list.append(pos[0] + card[0]/2 - description_width_list[k]/2)
                        y_list.append(pos[1]+(card[1]/2)+12.5*(len(description_list)-2)-25*k)

                    page.setFont('Meiryo UI', 25)  
                    for k in range(len(description_list)):
                        page.drawString(x_list[k],y_list[k], description_list[k])
...
if __name__ == "__main__":
    generate_description_pdf("/Users/masataka/Desktop/写真展フォーム　テンプレート.xlsx","/Users/masataka/Desktop/Data")
