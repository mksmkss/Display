...
def get_plates_list(excel_path):
    ...
    title_list = pd.read_excel(excel_path, index_col=0, usecols=[10]).index
    penname_list = pd.read_excel(excel_path, index_col=0, usecols=[12]).index
    ...
    while k < len(name_list):
        ...
        for i in range(works_num):
            ...
            if len(toArray(penname_list[k]))!=works_num:
                ...
                plates_list.append([toArray(title_list[k])[i], toArray(penname_list[k])[i]])
